...
import com.jscheng.spluto.markdown.core.bean.TextLinePiece;
import com.jscheng.spluto.markdown.core.bean.ValuePart;
import com.jscheng.spluto.markdown.core.parser.filter.CodePartFilter;
import com.jscheng.spluto.markdown.core.parser.filter.HeaderNextLineFilter;
import com.jscheng.spluto.markdown.core.parser.filter.HeaderOneLineFilter;
import com.jscheng.spluto.markdown.core.parser.filter.ListFilter;
import com.jscheng.spluto.markdown.core.parser.filter.SyntaxFilter;
import com.jscheng.spluto.markdown.core.parser.filter.TablePartFilter;
...
public class Analyzer {
	private static List<String> mdTokenInLine = Arrays.asList(
			MDToken.BOLD_WORD1,
			MDToken.BOLD_WORD2,
			MDToken.ITALIC_WORD,
			MDToken.ITALIC_WORD_2,
			MDToken.STRIKE_WORD,
			MDToken.CODE_WORD,
			MDToken.IMG,
			MDToken.LINK
	...
	public static List<ValuePart> analyzeLineText(String text) {
		...
		if(text.endsWith(MDToken.ROW)) {
		    result.add(new ValuePart("", TextType.NONE));
		}
		return result;
	}
	...
	private static List<ValuePart> text2ValuePart(String text, List<String> notCheckMDTokens,
		...
		List<ValuePart> result = new ArrayList<>();
		text = SymbolUtil.convertValue(text);
	...
	/**
	 * 检查mdtoken 对应的语法是否完整: LINK、IMG、COMMON
	 * @param i 开始查找的位置
	...
	private static TextLinePiece checkIfCorrectSyntax(int i, String mdToken, String text) {
		...
		if (mdToken.equals(MDToken.LINK) || mdToken.equals(MDToken.IMG)) {
			...
			if(j > -1) {
				textLinePiece = new TextLinePiece(i, j, TextLinePiece.PieceType.NONE);
			}
		}
	...
	private static ValuePart createValuePart(String value, List<String> mdTokens) {
		...
		if (mdTokens.size() > 0) {
			List<TextType> types = new ArrayList<>();
			int i = 0;
			for (int k = (mdTokens.size() - 1); k >= 0; k--) {
				types.add(TextType.convert(mdTokens.get(k))); // 这里引入i，是为了数组反序
				i++;
			}
