package com.jscheng.spluto.core.parser.filter;

import com.jscheng.spluto.core.bean.TextOrBlock;
import com.jscheng.spluto.core.parser.MDToken;
import com.jscheng.spluto.core.parser.builder.CodeBuilder;
		...
		for (int idx = 0, si = lines.size(); idx < si; idx++) {
			...
			if (codePartType != CODE_PART_TYPE_NONE) { // 检测到有代码片段
				...
				if (isCodeEnd) { // 确定有代码片段
					...
					textOrBlocks.add(new TextOrBlock(new CodeBuilder(interText.toString()).build()));
